...
export default function DashboardPage() {
  ...
  const [bacDate, setBacDate] = useState<string | undefined>();
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const bacRaw = await dashboardApi.getBacTime();
        const bacRes = bacRaw?.data?.data || bacRaw?.data || bacRaw;
        
        if (bacRes && bacRes?.bac_exam_date_start) {
          setBacDate(bacRes.bac_exam_date_start);
        }
      } catch (err: any) {
